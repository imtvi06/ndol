...
C CASO DE IOPT(19).EQ. 3

C ANULACION INICIAL DE LOS TERMINOS NO DIAGONALES EN LA MATRIZ DE ORDE-
C NES DE ENLACE MONOELECTRONICAS

C CASO DONDE LA MATRIZ DE DENSIDADES INICIAL SE ORIGINA EN
C LA COMPONENTE MONOELECTRONICA DEL HAMILTONIANO (DIAGONALIZACION DE B(I,J))
...
* CONSTRUCCION DE LA MATRIZ DE FOCK Y DIAGONALIZACION CON
* LA SUBRUTINA QRDIAG
...
*        FFF = (DN*DN)/(DNAUX*DNAUX)
        ...
        print *, FFF, AUEVI*(AII(NOCC+1)-AII(NOCC))
        DO 20 I=1,NA
          LI = NAT(I)
          MU = NO1(I)
          QII = LI.GT.2
...
*
1009   FORMAT (1X)
1020   FORMAT (' INITIAL GUESS DENSITY MATRIX OBTAINED BY A PROGRESSIVE 
     &METHOD')
      END
